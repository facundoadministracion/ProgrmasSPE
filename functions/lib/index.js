 ...
 * de forma atómica.
 */
exports.deleteParticipant = functions
    .region("southamerica-east1")
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "La función solo puede ser utilizada por un usuario autenticado.");
    }
    const userRole = context.auth.token.role;
    if (userRole !== "admin") {
        throw new functions.https.HttpsError("permission-denied", "Acción no permitida. Se requiere rol de administrador.");
    }
    const participantId = data.participantId;
    if (!participantId || typeof participantId !== "string") {
        throw new functions.https.HttpsError("invalid-argument", "El ID del participante es inválido o no fue proporcionado.");
    }
    try {
        ...
        throw new functions.https.HttpsError("internal", "Ocurrió un error inesperado al intentar eliminar los datos del participante.");
    }
});
